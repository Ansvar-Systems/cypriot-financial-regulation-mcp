/**
 * SQLite database access layer for the CySEC/CBC MCP server.
 *
 * Schema:
 *   - sourcebooks        — CySEC and CBC sourcebook directories
 *   - provisions         — individual directives, circulars, and prudential directives
 *   - enforcement_actions — CySEC enforcement decisions and sanctions
 *
...
export function getProvision(
  ...
  return (
    (db
      .prepare(
        "SELECT * FROM provisions WHERE sourcebook_id = ? AND LOWER(reference) = LOWER(?) LIMIT 1",
      )
      .get(sourcebook.toUpperCase(), reference) as Provision | undefined) ??
    null
  );
...
export function checkProvisionCurrency(reference: string): {
  ...
  const row = db
    .prepare(
      "SELECT reference, status, effective_date FROM provisions WHERE LOWER(reference) = LOWER(?) LIMIT 1",
    )
    .get(reference) as
